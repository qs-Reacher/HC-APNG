...
def AMI_calculator(Y_true, Y_pred):
    Y_pred_ = Y_pred[Y_pred != -1]
    Y_true_ = Y_true[Y_pred != -1]
    return adjusted_mutual_info_score(Y_true_, Y_pred_)


def FMI_calculator(Y_true, Y_pred):
    Y_pred_ = Y_pred[Y_pred != -1]
    Y_true_ = Y_true[Y_pred != -1]
    return fowlkes_mallows_score(Y_true_, Y_pred_)


def NMI_calculator(Y_true, Y_pred):
    Y_pred_ = Y_pred[Y_pred != -1]
    Y_true_ = Y_true[Y_pred != -1]
    return normalized_mutual_info_score(Y_true_, Y_pred_)
...
def measures_calculator(Y_true, Y_pred):
    ...
    df_mesures = pd.DataFrame(
        columns=['f1', 'ACC', 'AMI', 'ARI', 'FMI', 'NMI', 'cover_rate', 'classes'])

    f1 = f1_score_calculator(Y_true, Y_pred)
    ACC = ACC_calculator(Y_true, Y_pred)
    ARI = ARI_calculator(Y_true, Y_pred)
    AMI = AMI_calculator(Y_true, Y_pred)
    FMI = FMI_calculator(Y_true, Y_pred)
    NMI = NMI_calculator(Y_true, Y_pred)
    df_mesures.loc[0] = [f1, ACC, AMI, ARI, FMI, NMI, cover_rate, N_cls]
    print(
         " " + f"ACC:{ACC:.4f}" + " " + f"AMI:{AMI:.4f}" + " " + f"ARI:{ARI:.4f}"
        + " " + f"FMI:{FMI:.4f}" + " " + f"NMI:{NMI:.4f}")
    return df_mesures
